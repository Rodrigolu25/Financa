...
def dashboard():
    try:
        totals = {
            ...
            'despesas': float(db.session.query(func.sum(Despesa.valor)).filter(Despesa.ativo == True).scalar() or 0),
            'cartao': float(db.session.query(func.sum(CartaoCredito.valor)).filter(CartaoCredito.ativo == True).scalar() or 0),
            'donativos': float(db.session.query(func.sum(Donativo.valor)).filter(Donativo.ativo == True).scalar() or 0)
        }
        ...
        for model in [Ganho, Despesa, CartaoCredito, Donativo]:
            try:
                transactions.extend(model.query.filter(model.ativo == True).order_by(model.data.desc()).limit(5).all())
            except:
                continue
...
@app.route('/excluir/<tipo>/<int:id>', methods=['POST'])  # Corrigido aqui
def excluir_movimentacao(tipo, id):
    try:
        model = {
            ...
            'donativo': Donativo
        }.get(tipo.lower())  # Adicionei .lower() para garantir
